...
function buildRestrictedAccessExceptions(payload: any[]) {
  const rows = payload.map((request) => ({
    ...
    access_role: request.accessRole?.displayName ?? "",
    shared_drive: request.sharedDrive?.name ?? "",
    restricted_folder: request.restrictedFolder?.path ?? "",
    requested_access_level: request.requestedAccessLevel ?? "",
    other_target_text: request.otherTargetText ?? "",
    ...
    needed_by_date: request.neededByDate?.toISOString?.() ?? "",
    ...
    fulfilled_at: request.fulfilledAt?.toISOString?.() ?? "",
